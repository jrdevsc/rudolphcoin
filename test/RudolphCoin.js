...
it('Transfers ownership', function(){
  return RudolphCoin.deployed().then(function(instance){
    ...
    assert.equal(receipt.logs[0].event, 'Transfer', 'sould be the "transfer" event');
...
it('approves tokens for delegated transfer', function(){
  return RudolphCoin.deployed().then(function(instance){
    ...
    assert.equal(receipt.logs.length, 1, 'triggers one event');
    assert.equal(receipt.logs[0].event, 'Approval', 'sould be the "approval" event');
    assert.equal(receipt.logs[0].args._owner, accounts[0], 'logs the account tokens are transfered from');
    assert.equal(receipt.logs[0].args._spender, accounts[1], 'logs the account tokens are transfered to');
